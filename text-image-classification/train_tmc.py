...
from src.models import get_model,ce_loss
...
def get_args(parser):
    ...
    parser.add_argument("--model", type=str, default="tmc", choices=['tmc'])
...
def model_forward(i_epoch, model, args, criterion, batch):
    ...
    loss = ce_loss(tgt, txt_alpha, args.n_classes, i_epoch, args.annealing_epoch) + \
           ce_loss(tgt, img_alpha, args.n_classes, i_epoch, args.annealing_epoch) + \
           ce_loss(tgt, txt_img_alpha, args.n_classes, i_epoch, args.annealing_epoch)

    return loss, txt_img_alpha, tgt
